...
$(ARTIFACTS_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH):
	@mkdir -p $@

$(ARTIFACTS_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH)/s3ftpgateway$(SUFFIX): $(ARTIFACTS_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH) $(SRC_FILES)
	@echo " * Building binary for $(GOOS)/$(GOARCH)..."
	@CGO_ENABLED=0 go build -o $@ .

build: $(ARTIFACTS_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH)/s3ftpgateway$(SUFFIX)
...
release-targz: build $(RELEASE_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH)
	@echo " * Creating tar.gz for $(GOOS)/$(GOARCH)"
	tar -czf $(RELEASE_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH).tar.gz -C $(ARTIFACTS_DIR) s3ftpgateway_$(GOOS)_$(GOARCH)

release-zip: build $(RELEASE_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH)
	@echo " * Creating zip for $(GOOS)/$(GOARCH)"
	cd $(ARTIFACTS_DIR) && zip -9 $(RELEASE_DIR)/s3ftpgateway_$(GOOS)_$(GOARCH).zip s3ftpgateway_$(GOOS)_$(GOARCH)/*
